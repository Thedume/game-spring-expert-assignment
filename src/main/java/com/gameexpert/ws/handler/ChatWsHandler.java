...
public class ChatWsHandler implements WsMessageHandler {
    ...
    private ChatResponse createResponse(WsMessageContext context, String content) {
        ...
        ChatMessageResponse message = chatService.saveMessage(context.worldId(), context.nickname(), content);

        return new ChatResponse(message.getSender(), message.getContent(), message.getCreatedAt());
    }
}
